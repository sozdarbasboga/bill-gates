...
const Summary = ({ cart, balance }) => {
  const boughtProducts = Object.entries(cart).filter(([_, qty]) => qty > 0);
  const totalSpent = boughtProducts.reduce((sum, [id, qty]) => {
  ...
  return (
    <div style={{ background: '#f8f8f8', borderRadius: 12, padding: 24, margin: '32px auto 16px auto', maxWidth: 600 }}>
      ...
      {boughtProducts.length === 0 ? (
        <div style={{ color: '#888' }}>Henüz ürün alınmadı.</div>
      ) : (
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {boughtProducts.map(([id, qty]) => {
            const product = products.find(p => p.id === Number(id));
            return product ? (
